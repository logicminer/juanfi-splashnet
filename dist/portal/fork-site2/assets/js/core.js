...
$(document).ready(function(){
  ...
  $('#insertCoinModal').on('hidden.bs.modal', function () {
		...
		if(totalCoinReceived == 0){
			$.ajax({
			  type: "POST",
			  url: location.protocol+"//"+vendorIpAddress+"/cancelTopUp",
...
function populatePromoRates(retryCount){
	$.ajax({
	  type: "GET",
	  url: location.protocol+"//"+vendorIpAddress+"/getRates?rateType="+rateType+"&date="+(new Date().getTime()),
	  crossOrigin: true,
...
function populateChargingStations(retryCount){
	...
	$.ajax({
	  type: "GET",
	  url: location.protocol+"//"+vendorIpAddress+"/getChargingStation?date="+(new Date().getTime()),
	  crossOrigin: true,
	  contentType: 'text/plain',
	  success: function(data){
		var rows = data.split("|");
...
function addChargerTime(port, portName, retryCount){
	...
	$.ajax({
	  type: "POST",
	  url: location.protocol+"//"+vendorIpAddress+"/topUp",
...
function saveVoucherBtnAction(){
	...
	$.ajax({
	  type: "POST",
	  url: location.protocol+"//"+vendorIpAddress+"/useVoucher",
...
function checkCoin(){
	$.ajax({
	  type: "POST",
	  url: location.protocol+"//"+vendorIpAddress+"/checkCoin",
	  data: "voucher="+voucher,
	  success: function(data){
...
function convertVoucherAction(){
	...
	if(vc != ""){
		...
		$.ajax({
			type: "POST",
			url: location.protocol+"//"+vendorIpAddress+"/convertVoucher",
